          ...
          Callback<ICoreWebView2CreateCoreWebView2ControllerCompletedHandler>(
            [hwnd, wurl, activeId, env](HRESULT result, ICoreWebView2Controller* controller)->HRESULT
            {
              ...
              if (localWebView)
              {
                  ...
                  auto gotCb = Microsoft::WRL::Callback<ICoreWebView2FocusChangedEventHandler>(
                    [rec](ICoreWebView2Controller* /*sender*/, IUnknown* /*args*/) -> HRESULT {
                      for (auto& entry : g_instances) {
                        if (entry.second) entry.second->browserHasFocus = false;
                      }
                      rec->browserHasFocus = true;
                ...
                if (rec && rec->controller) {
                  ...
                  rec->controller->add_AcceleratorKeyPressed(Callback<ICoreWebView2AcceleratorKeyPressedEventHandler>(
                    [rec](ICoreWebView2Controller* /*sender*/, ICoreWebView2AcceleratorKeyPressedEventArgs* args)->HRESULT {
                      ...
                      LogF("[Hotkeys][webview2] id='%s' key=%u kind=%d global=%d instanceGate=%d editing=%d find=%d",
                           rec->id.c_str(), key, (int)kind,
                           (int)g_webViewSettings.forwardReaperHotkeys,
                           (int)rec->forwardReaperHotkeys,
                           (int)rec->webContentEditing,
                           (int)rec->findFieldFocused);
                      INT keyEventLParam=0; args->get_KeyEventLParam(&keyEventLParam); // modifiers not directly exposed; use GetKeyState as fallback
                      bool ctrl = (GetKeyState(VK_CONTROL)&0x8000)!=0;
                      bool shift = (GetKeyState(VK_SHIFT)&0x8000)!=0;
                      if (ctrl && (key=='F' || key=='f')) {
                        // Mark handled to suppress default dialog
                        args->put_Handled(TRUE);
                        WinRememberPageFocus(rec);
                        // Show or navigate find bar
                        if (!rec->showFindBar) {
                          rec->showFindBar = true; rec->findFieldFocused = true; LogRaw("[AccelCtrlF] show find bar");
                          bool titleVisible = (rec->titleBar && IsWindow(rec->titleBar) && IsWindowVisible(rec->titleBar));
                      ...
                      MSG forwarded{};
                      forwarded.hwnd = rec->hwnd;
                      forwarded.message = kind == COREWEBVIEW2_KEY_EVENT_KIND_SYSTEM_KEY_DOWN
                                                ? WM_SYSKEYDOWN
                                                : WM_KEYDOWN;
                      forwarded.wParam = key;
                      forwarded.lParam = keyEventLParam;
                      if (RouteWindowsWebViewKeyToReaper(&forwarded)) {
                        args->put_Handled(TRUE);
                        PostMessage(forwarded.hwnd, forwarded.message,
                                    forwarded.wParam, forwarded.lParam);
                        LogF("[Hotkeys][webview2-route] id='%s' key=%u message=%u",
                             rec->id.c_str(), key, forwarded.message);
                      }
                      return S_OK;
                    }
                  ).Get(), &accelTok);
                ...
                {
                  std::wstring kFRZCtxJS = std::wstring(L"window.__rwvForwardKeys=") +
                    ((g_webViewSettings.forwardReaperHotkeys && rec->forwardReaperHotkeys)
                       ? L"true;" : L"false;") + LR"JS(
                    (function(){
                      function isEditable(el) {
                        for (var cur = el; cur; ) {
                          if (cur.nodeType === 1) {
                            ...
                            if ((tag === 'textarea' || tag === 'select') && !cur.disabled) return true;
                            if (tag === 'input' && !cur.disabled && !cur.readOnly) {
                              var type = (cur.type || 'text').toLowerCase();
                              if (['button','checkbox','color','file','hidden','image','radio','range','reset','submit'].indexOf(type) < 0)
                                return true;
                            }
                      ...
                      function editingFromEvent(event) {
                        var path = event && event.composedPath ? event.composedPath() : [];
                        for (var i = 0; i < path.length; ++i)
                          if (isEditable(path[i])) return true;
                        return isEditable(deepActiveElement());
                      }
                      function reportEditingFocus(event) {
                        var focused = deepActiveElement();
                        if (editingFromEvent(event) && focused)
                          window.__rwvLastEditableFocus = focused;
                        if (window.chrome && window.chrome.webview)
                          window.chrome.webview.postMessage('EDIT|' + (editingFromEvent(event) ? '1' : '0'));
                      }
                      window.addEventListener('focusin', reportEditingFocus, true);
                      window.addEventListener('focusout', function(){ setTimeout(reportEditingFocus, 0); }, true);
                      window.addEventListener('blur', function(){
                        if (window.chrome && window.chrome.webview) window.chrome.webview.postMessage('EDIT|0');
                      }, true);
                      window.addEventListener('keydown', function(event) {
                        if (!window.__rwvForwardKeys || event.defaultPrevented || event.isComposing || editingFromEvent(event))
                          return;
                        // event.key is layout-dependent (for example KeyC is
                        // reported as Cyrillic с). event.code identifies the
                        // physical shortcut key consistently across layouts.
                        var code = event.code || '';
                        var key = code.indexOf('Key') === 0
                          ? code.substring(3).toLowerCase()
                          : (event.key || '').toLowerCase();
                        var primary = event.ctrlKey || event.metaKey;
                        if (primary && ['a','c','x'].indexOf(key) >= 0) {
                          if (key === 'a') {
                            document.execCommand('selectAll');
                          } else {
                            var selected = window.getSelection ? window.getSelection().toString() : '';
                            if (selected && window.chrome && window.chrome.webview)
                              window.chrome.webview.postMessage('CLIPBOARD_COPY|' + selected);
                            if (key === 'x' && selected) document.execCommand('delete');
                          }
                          event.preventDefault();
                          event.stopImmediatePropagation();
                          if (window.chrome && window.chrome.webview)
                            window.chrome.webview.postMessage('EDITKEY|' + key);
                          return;
                        }
                        if (primary && ['f','v','y','z'].indexOf(key) >= 0)
                          return;
                        var keyCode = event.keyCode || event.which || 0;
                        if (!keyCode || !window.chrome || !window.chrome.webview) return;
                        window.chrome.webview.postMessage('KEY|' + keyCode + '|' +
                          (event.altKey ? 1 : 0) + '|' + (event.ctrlKey ? 1 : 0) + '|' +
                          (event.shiftKey ? 1 : 0));
                        event.preventDefault();
                        event.stopImmediatePropagation();
                      }, true);
                    ...
                    window.addEventListener('contextmenu', function(e){
                      ...
                      if (focused) {
                        window.__rwvContextPreviousFocus = focused;
                        window.__rwvLastEditableFocus = focused;
                      }
                  ...
                  Callback<ICoreWebView2WebMessageReceivedEventHandler>(
                    [hwnd](ICoreWebView2*, ICoreWebView2WebMessageReceivedEventArgs* args)->HRESULT {
                      ...
                      if (SUCCEEDED(args->TryGetWebMessageAsString(&message)) && message) {
                        const std::wstring wideMessage(message.get());
                        std::string s = Narrow(wideMessage);
                        if (s.rfind("CTX|", 0) == 0) {
                          int sx=0, sy=0;
                          #ifdef _WIN32
                        ...
                        } else if (s == "EDIT|1" || s == "EDIT|0") {
                          WebViewInstanceRecord* rec = GetInstanceByHwnd(hwnd);
                          if (rec) {
                            rec->webContentEditing = s == "EDIT|1";
                            LogF("[WebEditFocus][win] id='%s' editing=%d",
                                 rec->id.c_str(), (int)rec->webContentEditing);
                          }
                        } else if (s.rfind("KEY|", 0) == 0) {
                          unsigned key=0; int alt=0, ctrl=0, shift=0;
                          if (sscanf_s(s.c_str()+4, "%u|%d|%d|%d", &key, &alt, &ctrl, &shift) == 4) {
                            WebViewInstanceRecord* rec = GetInstanceByHwnd(hwnd);
                            MSG forwarded{};
                            forwarded.hwnd = hwnd;
                            forwarded.message = alt ? WM_SYSKEYDOWN : WM_KEYDOWN;
                            forwarded.wParam = key;
                            if (rec && RouteWindowsWebViewKeyToReaper(&forwarded)) {
                              PostMessage(forwarded.hwnd, forwarded.message,
                                          forwarded.wParam, forwarded.lParam);
                              LogF("[Hotkeys][dom-route] id='%s' key=%u alt=%d ctrl=%d shift=%d",
                                   rec->id.c_str(), key, alt, ctrl, shift);
                            }
                          }
                        } else if (wideMessage.rfind(L"CLIPBOARD_COPY|", 0) == 0) {
                          const std::wstring text = wideMessage.substr(15);
                          if (!text.empty() && OpenClipboard(hwnd)) {
                            ...
                            CloseClipboard();
                            LogF("[EditCommand][win] copied selection chars=%llu",
                                 (unsigned long long)text.size());
                          }
                        } else if (s.rfind("EDITKEY|", 0) == 0) {
                          LogF("[Hotkeys][page-edit] id='%s' command='%s'",
                               GetInstanceByHwnd(hwnd) ? GetInstanceByHwnd(hwnd)->id.c_str() : "",
                               s.c_str() + 8);
                        }
                      }
                      return S_OK;
                ...
                localWebView->add_NavigationStarting(
                  Callback<ICoreWebView2NavigationStartingEventHandler>(
                    [activeId, hwnd](ICoreWebView2*, ICoreWebView2NavigationStartingEventArgs* args)->HRESULT
                    {
                      ...
                      WebViewInstanceRecord* r = GetInstanceById(activeId);
                      if (r) r->webContentEditing = false;
                      HWND target = (r && r->hwnd && IsWindow(r->hwnd)) ? r->hwnd : (IsWindow(hwnd)?hwnd:NULL);
                ...
                localWebView->add_NavigationCompleted(
                  Callback<ICoreWebView2NavigationCompletedEventHandler>(
                    [activeId, hwnd](ICoreWebView2* sender, ICoreWebView2NavigationCompletedEventArgs* args)->HRESULT
                    {
                      ...
                      if (ok && r && sender) {
                        ...
                        if (SUCCEEDED(sender->get_Source(&source)) && source) {
                          r->lastUrl = Narrow(std::wstring(source.get()));
                          SaveInstanceStateAll();
                          LogF("[NavigationState] id='%s' url='%s'",
                               r->id.c_str(), r->lastUrl.c_str());
                        }
                      }
                      HWND target = (r && r->hwnd && IsWindow(r->hwnd)) ? r->hwnd : (IsWindow(hwnd)?hwnd:NULL);
                ...
                localWebView->add_SourceChanged(
                  Callback<ICoreWebView2SourceChangedEventHandler>(
                    [activeId](ICoreWebView2* sender, ICoreWebView2SourceChangedEventArgs*)->HRESULT
                    {
                      WebViewInstanceRecord* r = GetInstanceById(activeId);
                      if (!r || !sender) return S_OK;
                      wil::unique_cotaskmem_string source;
                      if (SUCCEEDED(sender->get_Source(&source)) && source) {
                        r->lastUrl = Narrow(std::wstring(source.get()));
                        SaveInstanceStateAll();
                        LogF("[SourceChanged] id='%s' url='%s'",
                             r->id.c_str(), r->lastUrl.c_str());
                      }
                      return S_OK;
                    }).Get(), nullptr);
  ...
  if (rec->environment) {
    ...
    if (SUCCEEDED(rec->environment->QueryInterface(IID_PPV_ARGS(&env15))) && env15) {
      ...
      if (SUCCEEDED(hrC) && fresh) {
        ...
        const std::wstring wq = Widen(rec->findQuery);
...
void WinUpdateShortcutForwarding(WebViewInstanceRecord* rec)
{
  if (!rec || !rec->webview) return;
  const wchar_t* value = g_webViewSettings.forwardReaperHotkeys && rec->forwardReaperHotkeys
                           ? L"true" : L"false";
  std::wstring script = std::wstring(L"window.__rwvForwardKeys=") + value + L";";
  rec->webview->ExecuteScript(script.c_str(), nullptr);
}

void WinRememberPageFocus(WebViewInstanceRecord* rec)
{
  if (!rec || !rec->webview) return;
  rec->webview->ExecuteScript(
      L"(function(){var e=document.activeElement;"
      L"while(e&&e.shadowRoot&&e.shadowRoot.activeElement)e=e.shadowRoot.activeElement;"
      L"if((!e||e===document.body||e===document.documentElement)&&"
      L"window.__rwvLastEditableFocus&&window.__rwvLastEditableFocus.isConnected)"
      L"e=window.__rwvLastEditableFocus;"
      L"window.__rwvFindPreviousFocus=e;return true;})()",
      Callback<ICoreWebView2ExecuteScriptCompletedHandler>(
          [](HRESULT error, PCWSTR) -> HRESULT {
            if (FAILED(error))
              LogF("[FindFocus] remember failed: 0x%lX", (long)error);
            return S_OK;
          }).Get());
}

void WinRestoreContextMenuFocus(WebViewInstanceRecord* rec)
{
  if (!rec || !rec->webview) return;
  rec->webview->ExecuteScript(
      L"(function(){var e=window.__rwvContextPreviousFocus||window.__rwvLastEditableFocus;"
      L"window.__rwvContextPreviousFocus=null;"
      L"if(e&&e.isConnected&&typeof e.focus==='function'){e.focus({preventScroll:true});return true;}"
      L"return false;})()",
      Callback<ICoreWebView2ExecuteScriptCompletedHandler>(
          [](HRESULT error, PCWSTR result) -> HRESULT {
            if (FAILED(error))
              LogF("[ContextFocus][win] restore failed: 0x%lX", (long)error);
            else
              LogF("[ContextFocus][win] restore result=%ls", result ? result : L"null");
            return S_OK;
          }).Get());
}

void WinRestorePageFocus(WebViewInstanceRecord* rec)
{
  if (!rec || !rec->webview) return;
  rec->webview->ExecuteScript(
      L"(function(){var e=window.__rwvFindPreviousFocus||document.activeElement;"
      L"window.__rwvFindPreviousFocus=null;"
      L"if(e&&e.isConnected&&typeof e.focus==='function'){e.focus({preventScroll:true});return true;}"
      L"return false;})()",
      Callback<ICoreWebView2ExecuteScriptCompletedHandler>(
          [](HRESULT error, PCWSTR result) -> HRESULT {
            if (FAILED(error))
              LogF("[FindFocus] restore failed: 0x%lX", (long)error);
            else
              LogF("[FindFocus] restore result=%ls", result ? result : L"null");
            return S_OK;
          }).Get());
}
...
bool ExecuteWebViewEditCommand(WebViewInstanceRecord* rec, WebViewEditCommand command)
{
  ...
  std::wstring script;
  const wchar_t* focusPrefix =
      L"(function(){var e=window.__rwvContextPreviousFocus||window.__rwvLastEditableFocus||document.activeElement;"
      L"while(e&&e.shadowRoot&&e.shadowRoot.activeElement)e=e.shadowRoot.activeElement;"
      L"if(e&&e.isConnected&&typeof e.focus==='function')e.focus({preventScroll:true});";
  switch (command) {
    case WebViewEditCommand::Copy:
      script = std::wstring(focusPrefix) +
               L"var t='';if(e&&typeof e.value==='string'&&typeof e.selectionStart==='number')"
               L"t=e.value.slice(e.selectionStart,e.selectionEnd);"
               L"if(!t&&window.getSelection)t=window.getSelection().toString();"
               L"if(!t)return false;window.chrome.webview.postMessage('CLIPBOARD_COPY|'+t);return true;})()";
      break;
    case WebViewEditCommand::Cut:
      script = std::wstring(focusPrefix) +
               ...
               L"t=e.value.slice(e.selectionStart,e.selectionEnd);"
               L"if(!t&&window.getSelection)t=window.getSelection().toString();"
               L"if(!t)return false;window.chrome.webview.postMessage('CLIPBOARD_COPY|'+t);"
               L"if(e&&typeof e.value==='string'&&typeof e.selectionStart==='number'){"
               L"var a=e.selectionStart,b=e.selectionEnd;e.value=e.value.slice(0,a)+e.value.slice(b);"
               L"e.selectionStart=e.selectionEnd=a;e.dispatchEvent(new Event('input',{bubbles:true}));return true;}"
               L"return document.execCommand('delete');})()";
      break;
    case WebViewEditCommand::Paste: {
      if (!OpenClipboard(rec->hwnd)) return false;
      HANDLE data = GetClipboardData(CF_UNICODETEXT);
      const wchar_t* clipboardText = data ? static_cast<const wchar_t*>(GlobalLock(data)) : nullptr;
      if (!clipboardText) {
        CloseClipboard();
        return false;
      }
      std::wstring escaped;
      for (const wchar_t* p = clipboardText; *p; ++p) {
        switch (*p) {
      ...
      GlobalUnlock(data);
      CloseClipboard();
      script = L"(function(t){var e=window.__rwvContextPreviousFocus||window.__rwvLastEditableFocus||document.activeElement;"
               L"while(e&&e.shadowRoot&&e.shadowRoot.activeElement)e=e.shadowRoot.activeElement;"
               L"if(!e)return false;if(typeof e.focus==='function')e.focus({preventScroll:true});"
               L"if(document.execCommand('insertText',false,t))return true;"
               L"if('value' in e){var a=e.selectionStart||0,b=e.selectionEnd||a;"
               L"e.value=e.value.slice(0,a)+t+e.value.slice(b);"
               L"e.selectionStart=e.selectionEnd=a+t.length;"
               L"e.dispatchEvent(new Event('input',{bubbles:true}));return true;}return false;}('" +
               escaped + L"'))";
      break;
    }
  }

  return SUCCEEDED(rec->webview->ExecuteScript(
      script.c_str(),
      Callback<ICoreWebView2ExecuteScriptCompletedHandler>(
          [](HRESULT error, PCWSTR) -> HRESULT {
            if (FAILED(error)) LogF("[EditCommand] ExecuteScript failed: 0x%lX", (long)error);
            return S_OK;
          }).Get()));
}

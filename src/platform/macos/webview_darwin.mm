...
static NSMapTable<WKWebView*, NSResponder*>* g_contextResponders = nil;

static WebViewInstanceRecord* FindInstanceForWebView(WKWebView* webView)
{
  for (auto& entry : g_instances) {
    WebViewInstanceRecord* rec = entry.second.get();
    if (rec && rec->webView == webView) return rec;
  }
  return nullptr;
}
...
{
  WebViewInstanceRecord* rec = FindInstanceForWebView(webView);
  if (rec) rec->webContentEditing = false;
}
- (void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation
{
  ...
  if (currentURL) {
    rec->lastUrl = currentURL.UTF8String ?: "";
    SaveInstanceStateAll();
    LogF("[NavigationState][mac] id='%s' url='%s'",
         rec->id.c_str(), rec->lastUrl.c_str());
  }
  if (rec->hwnd) UpdateTitlesExtractAndApply(rec->hwnd);
  rec->findLastHighlightedQuery.clear();
  rec->findLastHighlightedCase = false;
  LogF("[Find][mac-fast] nav finish -> reset cache id='%s'", rec->id.c_str());
}
- (void)userContentController:(WKUserContentController *)userContentController
      didReceiveScriptMessage:(WKScriptMessage *)message
{
  if ([message.name isEqualToString:@"frzEdit"]) {
    WebViewInstanceRecord* rec = FindInstanceForWebView(message.webView);
    if (rec) {
      rec->webContentEditing = [message.body respondsToSelector:@selector(boolValue)] &&
                               [message.body boolValue];
      LogF("[WebEditFocus][mac] id='%s' editing=%d",
           rec->id.c_str(), (int)rec->webContentEditing);
    }
    return;
  }
  ...
  WebViewInstanceRecord* rec = FindInstanceForWebView(message.webView);
  if (rec && rec->hwnd) {
    MacRememberContextMenuFocus(rec);
    PostMessage(rec->hwnd, WM_CONTEXTMENU, (WPARAM)rec->hwnd, MAKELPARAM(sx, sy));
  }
}
@end
...
void StartWebView(HWND hwnd, const std::string& initial_url)
{
  ...
  // JS hook for right-click. Text selection remains enabled for Copy/Cut.
    ...
    "function isEditable(el){"
      "for(var cur=el;cur;){"
        "if(cur.nodeType===1){"
          ...
          "if((t==='textarea'||t==='select')&&!cur.disabled)return true;"
          "if(t==='input'&&!cur.disabled&&!cur.readOnly){"
            "var ty=(cur.type||'text').toLowerCase();"
            "if(['button','checkbox','color','file','hidden','image','radio','range','reset','submit'].indexOf(ty)<0)return true;"
          "}"
          "if((cur.getAttribute('role')||'').toLowerCase()==='textbox')return true;"
        "}"
      ...
      "return false;"
    "}"
    "function deepActiveElement(){var el=document.activeElement;while(el&&el.shadowRoot&&el.shadowRoot.activeElement)el=el.shadowRoot.activeElement;return el;}"
    "function editingFromEvent(e){var path=e&&e.composedPath?e.composedPath():[];for(var i=0;i<path.length;i++)if(isEditable(path[i]))return true;return isEditable(deepActiveElement());}"
    "function reportEdit(e){try{window.webkit.messageHandlers.frzEdit.postMessage(editingFromEvent(e));}catch(_){}}"
    "window.addEventListener('focusin',reportEdit,true);"
    "window.addEventListener('focusout',function(){setTimeout(reportEdit,0);},true);"
    "window.addEventListener('blur',function(){try{window.webkit.messageHandlers.frzEdit.postMessage(false);}catch(_){}},true);"
    "reportEdit();"
  ...
  if(!s_focusHooksInstalled){
    ...
    // Route browser editing commands through the actual WKContentView first
    // responder. REAPER's SWELL accelerator path does not preserve Cocoa edit
    // selectors when a plug-in-owned window is focused.
    [NSEvent addLocalMonitorForEventsMatchingMask:NSEventMaskKeyDown handler:^NSEvent*(NSEvent* e){
      if((e.modifierFlags & NSEventModifierFlagCommand) && !((e.modifierFlags & NSEventModifierFlagOption))){
        // NSEvent keyCode identifies the physical key and therefore keeps
        // Cmd+A/C/X/V/F/Z/Y working in non-Latin keyboard layouts.
        unichar c = 0;
        switch (e.keyCode) {
          case 0:  c = 'a'; break;
          case 3:  c = 'f'; break;
        ...
        if(c){
          NSWindow* eventWindow = e.window ?: [NSApp keyWindow];
          NSResponder* responder = eventWindow ? [eventWindow firstResponder] : nil;
          ...
          if ([responder isKindOfClass:[NSView class]]) {
            ...
            if (view) rec = FindInstanceForWebView((WKWebView*)view);
          }
          if (!rec && !g_activeInstanceId.empty()) {
            WebViewInstanceRecord* active = GetInstanceById(g_activeInstanceId);
            if (active && active->showFindBar && active->findFieldFocused &&
                active->findBarView && [active->findBarView window] == eventWindow)
              rec = active;
          }

          SEL editAction = nullptr;
          if(c=='a' || c=='A') editAction = @selector(selectAll:);
          else if(c=='c' || c=='C') editAction = @selector(copy:);
          else if(c=='x' || c=='X') editAction = @selector(cut:);
          else if(c=='v' || c=='V') editAction = @selector(paste:);
          else if(c=='y' || c=='Y') editAction = @selector(redo:);
          else if(c=='z' || c=='Z') editAction =
              (e.modifierFlags & NSEventModifierFlagShift) ? @selector(redo:) : @selector(undo:);

          if(rec && editAction && responder){
            const BOOL handled = [NSApp sendAction:editAction to:responder from:rec->webView];
            LogF("[Hotkeys][mac-event] key=%C action=%s handled=%d responder=%s",
                 c, sel_getName(editAction), (int)handled, object_getClassName(responder));
            if(handled) return (NSEvent*)nil;
          }

          if(c=='f' || c=='F'){
            if(rec){
              if(!rec->showFindBar){
                MacRememberPageFocus(rec);
...
static int g_urlObsSentinel = 0;
static void* kUrlObservationContext = &g_urlObsSentinel;
...
static void ObserveTitleIfNeeded(WKWebView* wv, HWND hwnd)
{
  ...
  [wv addObserver:wrap forKeyPath:@"URL" options:NSKeyValueObservingOptionNew context:kUrlObservationContext];
}
...
extern "C" void FRZ_RemoveTitleObserverFor(WKWebView* wv)
{
  ...
  if (it != g_kvoWrappers.end()) {
    ...
    @try { [wv removeObserver:wrap forKeyPath:@"URL" context:kUrlObservationContext]; } @catch(...) {}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context
{
  ...
  if (context == kUrlObservationContext) {
    ...
    if (rec && currentURL) {
      rec->lastUrl = currentURL.UTF8String ?: "";
      SaveInstanceStateAll();
      LogF("[SourceChanged][mac] id='%s' url='%s'",
           rec->id.c_str(), rec->lastUrl.c_str());
    }
    return;
  }
...
static void CleanupTitleObservers()
{
  for (auto &kv : g_kvoWrappers) {
    ...
    @catch(...) { }
    @try { [wv removeObserver:wrap forKeyPath:@"URL" context:kUrlObservationContext]; }
    @catch(...) { }
  }
  g_kvoWrappers.clear();
}
...
void MacRememberPageFocus(WebViewInstanceRecord* rec)
{
  if (!rec || !rec->webView) return;
  [rec->webView evaluateJavaScript:@"(function(){var e=document.activeElement;"
                                       "while(e&&e.shadowRoot&&e.shadowRoot.activeElement)e=e.shadowRoot.activeElement;"
                                       "window.__rwvFindPreviousFocus=e;return true;})()"
                 completionHandler:^(id, NSError* error) {
    if (error) LogF("[FindFocus][mac] remember failed: %s", error.localizedDescription.UTF8String);
  }];
}

void MacRestorePageFocus(WebViewInstanceRecord* rec)
{
  if (!rec || !rec->webView) return;
  WKWebView* webView = rec->webView;
  dispatch_async(dispatch_get_main_queue(), ^{
    NSWindow* window = webView.window;
    if (window) [window makeFirstResponder:webView];
    NSString* script = @"(function(){var e=window.__rwvFindPreviousFocus||document.activeElement;"
                        "window.__rwvFindPreviousFocus=null;"
                        "if(e&&e.isConnected&&typeof e.focus==='function'){e.focus({preventScroll:true});"
                        "return !!(e.isContentEditable||/^(INPUT|TEXTAREA|SELECT)$/.test(e.tagName));}"
                        "return false;})()";
    [webView evaluateJavaScript:script completionHandler:^(id result, NSError* error) {
      if (error)
        LogF("[FindFocus][mac] restore failed: %s", error.localizedDescription.UTF8String);
      else {
        const bool editing = [result respondsToSelector:@selector(boolValue)] && [result boolValue];
        WebViewInstanceRecord* current = FindInstanceForWebView(webView);
        if (current) current->webContentEditing = editing;
        LogF("[FindFocus][mac] restore complete editing=%d", (int)editing);
      }
    }];
  });
}

void MacRememberContextMenuFocus(WebViewInstanceRecord* rec)
{
  if (!rec || !rec->webView) return;
  if (!g_contextResponders)
    g_contextResponders = [[NSMapTable alloc]
        initWithKeyOptions:NSPointerFunctionsWeakMemory
          valueOptions:NSPointerFunctionsWeakMemory
              capacity:0];

  NSWindow* window = rec->webView.window;
  NSResponder* responder = window.firstResponder;
  if ([responder isKindOfClass:[NSView class]]) {
    NSView* view = (NSView*)responder;
    if ([view isDescendantOf:rec->webView] || view == rec->webView)
      [g_contextResponders setObject:responder forKey:rec->webView];
  }

  [rec->webView evaluateJavaScript:
      @"(function(){var e=document.activeElement;"
       "while(e&&e.shadowRoot&&e.shadowRoot.activeElement)e=e.shadowRoot.activeElement;"
       "window.__rwvContextPreviousFocus=e;return true;})()"
                       completionHandler:nil];
}

static NSResponder* RestoreContextMenuNativeResponder(WebViewInstanceRecord* rec)
{
  if (!rec || !rec->webView) return nil;
  NSResponder* responder = [g_contextResponders objectForKey:rec->webView];
  NSWindow* window = rec->webView.window;
  if (window && responder &&
      (![responder isKindOfClass:[NSView class]] ||
       [(NSView*)responder window] == window)) {
...
void MacRestoreContextMenuFocus(WebViewInstanceRecord* rec)
{
  if (!rec || !rec->webView) return;
  RestoreContextMenuNativeResponder(rec);
  WKWebView* webView = rec->webView;
  [webView evaluateJavaScript:
      @"(function(){var e=window.__rwvContextPreviousFocus;"
       "window.__rwvContextPreviousFocus=null;"
       "if(e&&e.isConnected&&typeof e.focus==='function'){"
         "e.focus({preventScroll:true});"
         "return !!(e.isContentEditable||/^(INPUT|TEXTAREA|SELECT)$/.test(e.tagName));}"
       "return false;})()"
            completionHandler:^(id result, NSError* error) {
    if (error) {
      LogF("[ContextFocus][mac] restore failed: %s",
           error.localizedDescription.UTF8String);
      return;
    }
    WebViewInstanceRecord* current = FindInstanceForWebView(webView);
    if (current && [result respondsToSelector:@selector(boolValue)])
      current->webContentEditing = [result boolValue];
  }];
}

bool ExecuteWebViewEditCommand(WebViewInstanceRecord* rec, WebViewEditCommand command)
{
  if (!rec || !rec->webView) return false;

  // AppKit's copy:/cut: dispatch works for WKWebView editors, but after our
  // custom context menu it does not reliably expose a plain DOM selection.
  // Read that selection in the page and put it on the native pasteboard so
  // Copy also works for ordinary page text (not only inputs/contenteditable).
  if (command == WebViewEditCommand::Copy || command == WebViewEditCommand::Cut) {
    const bool cut = command == WebViewEditCommand::Cut;
    WKWebView* webView = rec->webView;
    NSString* script = cut
        ? @"(function(){var e=document.activeElement;"
           "while(e&&e.shadowRoot&&e.shadowRoot.activeElement)e=e.shadowRoot.activeElement;"
           "var t='';if(e&&typeof e.value==='string'&&typeof e.selectionStart==='number'){"
             "var a=e.selectionStart,b=e.selectionEnd;t=e.value.slice(a,b);"
             "if(t){e.value=e.value.slice(0,a)+e.value.slice(b);e.selectionStart=e.selectionEnd=a;"
             "e.dispatchEvent(new Event('input',{bubbles:true}));}}"
           "if(!t&&window.getSelection)t=window.getSelection().toString();"
           "if(t&&!(e&&typeof e.value==='string'))document.execCommand('delete');return t;})()"
        : @"(function(){var e=document.activeElement;"
           "while(e&&e.shadowRoot&&e.shadowRoot.activeElement)e=e.shadowRoot.activeElement;"
           "var t='';if(e&&typeof e.value==='string'&&typeof e.selectionStart==='number')"
             "t=e.value.slice(e.selectionStart,e.selectionEnd);"
           "if(!t&&window.getSelection)t=window.getSelection().toString();return t;})()";
    [webView evaluateJavaScript:script completionHandler:^(id result, NSError* error) {
      if (error) {
        LogF("[EditCommand][mac] selection read failed: %s",
             error.localizedDescription.UTF8String);
        return;
      }
      ...
      [pasteboard setString:(NSString*)result forType:NSPasteboardTypeString];
      LogF("[EditCommand][mac] %s selection chars=%llu",
           cut ? "cut" : "copied", (unsigned long long)[(NSString*)result length]);
    }];
    return true;
  }
  ...
  SEL action = nullptr;
  switch (command) {
    case WebViewEditCommand::Copy:
    case WebViewEditCommand::Cut:
      return false;
    case WebViewEditCommand::Paste: action = @selector(paste:); break;
  }

  NSResponder* responder = RestoreContextMenuNativeResponder(rec);
  return responder && [NSApp sendAction:action to:responder from:rec->webView] == YES;
}

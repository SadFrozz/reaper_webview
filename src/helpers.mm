...
void SetTabTitleInplace(HWND hwnd, const std::string& tabCaption)
{
  ...
  if (!w.empty()) {
    ...
    if (n > 0) {
      std::string acp((size_t)n, '\0');
      WideCharToMultiByte(CP_ACP, 0, w.data(), (int)w.size(), acp.data(), n, nullptr, nullptr);
...
static inline bool LooksLikeHostWithPort(const std::string& s)
{
  const size_t colon = s.find(':');
  if (colon == std::string::npos || colon == 0 || colon + 1 >= s.size()) return false;

  // A hostname followed by a numeric port is not a URI scheme. This matters
  // for values such as "example.com:8443" and "localhost:3000".
  size_t portEnd = colon + 1;
  while (portEnd < s.size() && std::isdigit(static_cast<unsigned char>(s[portEnd])))
    ++portEnd;
  if (portEnd == colon + 1) return false;
  if (portEnd < s.size() && s[portEnd] != '/' && s[portEnd] != '?' && s[portEnd] != '#')
    return false;

  const std::string host = s.substr(0, colon);
  return host == "localhost" || host.find('.') != std::string::npos;
}
                            ...
                            std::string& outReason)
{
  ...
  // Protocol-relative web addresses use HTTPS as the safe default.
  if (s.rfind("//", 0) == 0) {
    outNormalized = std::string("https:") + s;
    outReason = "protocol_relative_https";
    return true;
  }

  if (LooksLikeHostWithPort(s)) {
    outNormalized = std::string("https://") + s;
    outReason = "host_port_prefixed_https";
    return true;
  }

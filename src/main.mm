...
//  - Named instances persist user-controlled state in REAPER's INI file.
...
static const UINT WM_RWV_FOCUS_WEBVIEW = WM_APP + 0x453;
...
static LRESULT CALLBACK RWVFindEditProc(HWND h, UINT m, WPARAM w, LPARAM l)
{
  switch(m){
    ...
    case WM_SETFOCUS: {
      LogRaw("[FindFocus] edit WM_SETFOCUS"); g_lastFindEdit = h; HWND host = GetParent(GetParent(h)); WebViewInstanceRecord* rec = GetInstanceByHwnd(host); if (rec) { rec->findFieldFocused = true; UpdateFocusChain(rec->id); } break; }
    case WM_KEYDOWN:
      if (w==VK_RETURN){
  ...
  if (rec){ bool fwd = !shift; g_findEnterActive = true; g_findLastEnterTick = GetTickCount(); WinFindNavigate(rec, fwd); SetFocus(h); SendMessageW(h, EM_SETSEL, (WPARAM)-1, (LPARAM)-1); HWND findBar = GetParent(h); if (findBar && IsWindow(findBar)) PostMessage(findBar, WM_RWV_FIND_REFOCUS, (WPARAM)h, 0); return 0; }
      ...
      if (w==VK_ESCAPE){
        HWND host = GetParent(GetParent(h)); WebViewInstanceRecord* rec = GetInstanceByHwnd(host);
        if (rec && rec->showFindBar) {
          rec->findFieldFocused = false;
          rec->showFindBar = false;
          LogRaw("[Find] close via Escape (Windows)");
          WinFindClose(rec);
          LayoutTitleBarAndWebView(host, rec->titleBar && IsWindowVisible(rec->titleBar));
          // Moving focus while the native edit control is still dispatching
          // VK_ESCAPE is unreliable. Defer it until this message completes so
          // WebView2 can restore the page's active HTML editor.
          PostMessage(host, WM_RWV_FOCUS_WEBVIEW, 0, 0);
          return 0;
        }
      }
      break;
    case WM_CHAR: if (w=='\r' || w==VK_ESCAPE) return 0; break;
    ...
    case WM_KILLFOCUS:
      ...
      { HWND host = GetParent(GetParent(h)); WebViewInstanceRecord* rec = GetInstanceByHwnd(host); if (rec) rec->findFieldFocused = false; }
      break;
  }
...
static LRESULT CALLBACK RWVHostSubclassProc(HWND h, UINT m, WPARAM w, LPARAM l)
{
  ...
  if (m == WM_RWV_FOCUS_WEBVIEW) {
    SetFocus(h);
    if (rec && rec->controller) {
      rec->controller->MoveFocus(COREWEBVIEW2_MOVE_FOCUS_REASON_PROGRAMMATIC);
      WinRestorePageFocus(rec);
    }
    return 0;
  }
  switch(m){
...
{
  WebViewInstanceRecord* rec = GetInstanceByHwnd(self.rwvHostHWND);
  if (rec) rec->findFieldFocused = true;
}
- (void)controlTextDidEndEditing:(NSNotification *)note
{
  WebViewInstanceRecord* rec = GetInstanceByHwnd(self.rwvHostHWND);
  if (rec) rec->findFieldFocused = false;
}
- (BOOL)control:(NSControl *)control textView:(NSTextView *)textView doCommandBySelector:(SEL)commandSelector
{
  ...
  if (commandSelector == @selector(cancelOperation:)) {
    ...
    if (rec && rec->showFindBar) {
      rec->findFieldFocused = false;
      rec->showFindBar = false;
      LogRaw("[Find] close via Escape (macOS)");
      [self setHidden:YES];
      LayoutTitleBarAndWebView(self.rwvHostHWND,
                              rec->titleBarView && ![rec->titleBarView isHidden]);
      NSWindow* window = [(NSView*)rec->webView window];
...
static void RememberWantDock(HWND hwnd)
{
  ...
  if (rec) {
    ...
    if (!rec->isRandomInstance) SaveInstanceDocked(rec->id, detected);
  }
...
{
  constexpr UINT kCmdDockToggle = 10001;
  constexpr UINT kCmdClose = 10099;
  constexpr UINT kCmdOpenUrl = 10105;
  constexpr UINT kCmdReload = 10110;
  constexpr UINT kCmdBack = 10111;
  constexpr UINT kCmdForward = 10112;
  constexpr UINT kCmdFind = 10113;
  constexpr UINT kCmdCopy = 10120;
  constexpr UINT kCmdCut = 10121;
  constexpr UINT kCmdPaste = 10122;
  constexpr UINT kCmdForwardHotkeys = 10130;
  ...
  AppendMenuA(m, MF_STRING, kCmdOpenUrl, "Open URL");
  AppendMenuA(m, MF_SEPARATOR, 0, NULL);

  if (!basicOnly) {
    // Reload / Back / Forward / Find stub
    AppendMenuA(m, MF_STRING, kCmdReload, "Reload");
...
#endif
    AppendMenuA(m, MF_STRING | (canBack?0:MF_DISABLED|MF_GRAYED), kCmdBack, "Back");
    AppendMenuA(m, MF_STRING | (canFwd?0:MF_DISABLED|MF_GRAYED), kCmdForward, "Forward");
    AppendMenuA(m, MF_SEPARATOR, 0, NULL);
    if (g_webViewSettings.showClipboardCommands) {
      AppendMenuA(m, MF_STRING, kCmdCopy, "Copy");
      AppendMenuA(m, MF_STRING, kCmdCut, "Cut");
      AppendMenuA(m, MF_STRING, kCmdPaste, "Paste");
      AppendMenuA(m, MF_SEPARATOR, 0, NULL);
    }
    AppendMenuA(m, MF_STRING, kCmdFind, "Find on page");
    AppendMenuA(m, MF_SEPARATOR, 0, NULL);
  }

  AppendMenuA(m, MF_STRING | (inDock?MF_CHECKED:0), kCmdDockToggle, inDock ? "Undock window" : "Dock window in Docker");
  if (rec && !rec->isRandomInstance) {
    AppendMenuA(m, MF_STRING | (rec->forwardReaperHotkeys ? MF_CHECKED : 0),
                kCmdForwardHotkeys, "Forward shortcuts to REAPER (this instance)");
  }
  AppendMenuA(m, MF_SEPARATOR, 0, NULL);
  AppendMenuA(m, MF_STRING, kCmdClose, "Close");
  ...
  DestroyMenu(m);
#ifdef _WIN32
  if (rec) WinRestoreContextMenuFocus(rec);
#elif defined(__APPLE__)
  if (rec) MacRestoreContextMenuFocus(rec);
#endif
  if (!cmd) return;
  ...
  if (cmd == kCmdOpenUrl) {
    ...
    SaveInstanceStateAll();
  }
  else if (cmd == kCmdReload) {
  ...
  else if (cmd == kCmdCopy || cmd == kCmdCut || cmd == kCmdPaste) {
    WebViewInstanceRecord* r = GetInstanceByHwnd(hwnd);
    WebViewEditCommand editCommand = WebViewEditCommand::Copy;
    if (cmd == kCmdCut) editCommand = WebViewEditCommand::Cut;
    if (cmd == kCmdPaste) editCommand = WebViewEditCommand::Paste;
    ExecuteWebViewEditCommand(r, editCommand);
  }
  else if (cmd == kCmdFind) {
    WebViewInstanceRecord* r = GetInstanceByHwnd(hwnd);
    if (r) {
  r->showFindBar = !r->showFindBar;
  if (!r->showFindBar) r->findFieldFocused = false;
...
#ifdef _WIN32
  if (r->showFindBar && r->findEdit) { r->findFieldFocused = true; SetFocus(r->findEdit); SendMessage(r->findEdit, EM_SETSEL, 0, -1); }
#else
  if (r->showFindBar && r->findEdit) { [((NSTextField*)r->findEdit) selectText:nil]; [[r->findEdit window] makeFirstResponder:((NSTextField*)r->findEdit)]; r->findFieldFocused = true; }
#endif
    } else { LogRaw("[Find] toggle requested but instance not found"); }
  }
  else if (cmd == kCmdForwardHotkeys) {
    WebViewInstanceRecord* r = GetInstanceByHwnd(hwnd);
    if (r && !r->isRandomInstance) {
      r->forwardReaperHotkeys = !r->forwardReaperHotkeys;
      SaveInstanceHotkeyForwarding(r->id, r->forwardReaperHotkeys);
#ifdef _WIN32
      WinUpdateShortcutForwarding(r);
#endif
      LogF("[Hotkeys] instance='%s' forwarding=%d (global=%d)",
           r->id.c_str(), (int)r->forwardReaperHotkeys,
           (int)g_webViewSettings.forwardReaperHotkeys);
    }
  }
  else if (cmd == kCmdClose) SendMessage(hwnd, WM_CLOSE, 0, 0);
}
...
static INT_PTR WINAPI WebViewDlgProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
  switch (msg)
  {
    ...
    case WM_INITDIALOG:
    {
        ...
        g_rwvMsgHook = SetWindowsHookExW(WH_GETMESSAGE, [](int code, WPARAM wP, LPARAM lP)->LRESULT {
          if (code >= 0) {
            ...
            if (m) {
              ...
              if (m->message==WM_KEYDOWN && (m->wParam=='F' || m->wParam=='f') && (GetKeyState(VK_CONTROL)&0x8000)) {
                ...
                if (host){
                  ...
                  if (rec){
                    ...
                    if (!rec->showFindBar){
                      rec->showFindBar = true; rec->findFieldFocused = true; LogRaw("[FindCtrlF] show find bar (Ctrl+F)");
              ...
              // WebView2 owns its child message queue, so docked and named
              // instances do not reliably reach REAPER's accelerator pass.
              // Retarget eligible key-down messages while preserving page and
              // find-field editing keys locally.
              RouteWindowsWebViewKeyToReaper(m);
            }
          }
      ...
      std::string url = (initial && *initial) ? std::string(initial) : g_webViewSettings.homePage;
      ...
      // Unknown state means a normal floating window. Docking is opt-in and is
      // remembered after the user docks the instance.
      const bool wantDock = (g_want_dock_on_create == 1);
      if (wantDock && DockWindowAddEx) {
        ...
        if (recInit && recInit->hasWindowPlacement) {
          SetWindowPos(hwnd, NULL,
                       recInit->windowX, recInit->windowY,
                       recInit->windowWidth, recInit->windowHeight,
    ...
    case WM_COMMAND:
      switch (LOWORD(wp))
      {
        case IDC_FIND_CLOSE:
        {
          WebViewInstanceRecord* r = GetInstanceByHwnd(hwnd); if (r && r->showFindBar){ r->findFieldFocused=false; r->showFindBar=false; LogRaw("[Find] close");
#ifdef _WIN32
            WinFindClose(r);
#endif
        #ifdef _WIN32
            ...
            LayoutTitleBarAndWebView(hwnd, titleVisible);
#ifdef _WIN32
            PostMessage(hwnd, WM_RWV_FOCUS_WEBVIEW, 0, 0);
#else
            NSWindow* window = [(NSView*)r->webView window];
            if (window && r->webView) {
              [window makeFirstResponder:r->webView];
              MacRestorePageFocus(r);
            }
#endif
          }
          return 0;
        ...
        case IDC_FIND_EDIT:
        {
          if (HIWORD(wp)==EN_CHANGE) {
            ...
            if (r && r->findEdit){
              ...
              // Convert to UTF-8
              r->findQuery = Narrow(std::wstring(wbuf));
    ...
    case WM_CLOSE: {
      ...
      for (auto &kv : g_instances) {
        if (kv.second && kv.second->hwnd == hwnd) {
          bool floating = false;
          const bool docked = DockIsChildOfDock && DockIsChildOfDock(hwnd, &floating) >= 0;
          if (!docked) {
            RECT placement{};
...
#endif
            if (!havePlacement) havePlacement = GetWindowRect(hwnd, &placement) != 0;
            if (havePlacement) {
              kv.second->windowX = placement.left;
              kv.second->windowY = placement.top;
              kv.second->windowWidth = placement.right - placement.left;
              kv.second->windowHeight = placement.bottom - placement.top;
              kv.second->hasWindowPlacement = kv.second->windowWidth > 0 && kv.second->windowHeight > 0;
            }
          }
          closedId = kv.first;
          ...
          DestroyTitleBarResources(kv.second.get());
      ...
      // WM_CLOSE is a user-visible close operation. Persist the remaining live
      // set now. REAPER's own shutdown destroys extension windows directly via
      // WM_DESTROY, so that path intentionally does not erase this snapshot.
      SaveInstanceStateAll();
    ...
    case WM_DESTROY: {
      LogRaw("[WM_DESTROY]");
      // WM_CLOSE normally clears the record first. This also handles windows
      // destroyed externally by REAPER or while the plug-in is unloading.
      for (auto &kv : g_instances) {
        WebViewInstanceRecord* rec = kv.second.get();
        if (!rec || rec->hwnd != hwnd) continue;
        DestroyTitleBarResources(rec);
        rec->hwnd = nullptr;
      #ifdef _WIN32
        if (rec->bmpPrev){ DeleteObject(rec->bmpPrev); rec->bmpPrev=nullptr; }
        if (rec->bmpNext){ DeleteObject(rec->bmpNext); rec->bmpNext=nullptr; }
...
void OpenOrActivateInstance(const std::string& instanceId, const std::string& url)
{
  ...
  if (hwnd) {
    // WM_INITDIALOG can bind rec->hwnd before CreateNewWebViewWindow returns.
    // Always retain the requested URL so the live snapshot written below is
    // complete, including immediately after a startup restoration.
    rec->hwnd = hwnd;
    if (!url.empty()) rec->lastUrl = url;
    rec->wantDockOnCreate = g_want_dock_on_create;
  }
#ifdef _WIN32
...
// ============================== Hook command ==============================
static bool g_shutdownInstanceSnapshotSaved = false;

static bool HookCommandProc(int cmd, int flag)
{
  // REAPER closes extension-owned child windows before unloading extensions.
  // Capture the live set while the windows still exist, then preserve that
  // snapshot during the later unload callback.
  if (cmd == 40004 && !g_shutdownInstanceSnapshotSaved) { // File: Quit REAPER
    CancelInstanceStateRestore();
    SaveInstanceStateAll();
    g_shutdownInstanceSnapshotSaved = true;
    LogRaw("[Restore] captured live instance snapshot before REAPER quit");
  }
  if (cmd == g_command_id) {
    OpenOrActivateInstance("wv_default", g_webViewSettings.homePage.c_str());
    return true; }
...
REAPER_PLUGIN_ENTRYPOINT(REAPER_PLUGIN_HINSTANCE hInstance, reaper_plugin_info_t* rec)
{
  ...
  if (rec)
  {
    ...
    LoadWebViewSettings();
    LoadInstanceStateAll();
    RegisterCommandId();
    RegisterAPI();
    RegisterHotkeyBridge();
  ...
  else
  {
    ...
    CancelInstanceStateRestore();
    // REAPER destroys extension windows before unloading the plug-in when the
    // main window is closed. The live snapshot is maintained as instances are
    // opened, changed or manually closed; never overwrite it here with the
    // already-destroyed (empty) runtime set.
    LogRaw("[Restore] preserving latest live instance snapshot on unload");
    UnregisterCommandId();
    UnregisterAPI();
    UnregisterHotkeyBridge();
    UnregisterPreferencesPage();

#ifdef _WIN32
    if (g_rwvMsgHook) {
      UnhookWindowsHookEx(g_rwvMsgHook);
      g_rwvMsgHook = nullptr;
      LogRaw("[FindHook] removed WH_GETMESSAGE on plugin unload");
    }
#endif
...
static bool Act_Search(int /*flag*/)
{
  ...
  if (!rec->showFindBar) {
    ...
    rec->showFindBar = true; rec->findFieldFocused = true; bool titleVisible = (rec->titleBar && IsWindow(rec->titleBar) && IsWindowVisible(rec->titleBar));
  ...
  if (!rec->showFindBar) {
    ...
    rec->findFieldFocused = true;
    LayoutTitleBarAndWebView(rec->hwnd, rec->titleBarView && ![rec->titleBarView isHidden]);
    EnsureFindBarCreated(rec->hwnd);
    if(rec->findEdit){ [(NSTextField*)rec->findEdit selectText:nil]; [[(NSTextField*)rec->findEdit window] makeFirstResponder:(NSTextField*)rec->findEdit]; rec->findFieldFocused = true; }
    LogRaw("[FindAction][mac] show via action list");
  } else {
    EnsureFindBarCreated(rec->hwnd);
    if(rec->findEdit){ [[(NSTextField*)rec->findEdit window] makeFirstResponder:(NSTextField*)rec->findEdit]; rec->findFieldFocused = true; }
...
static INT_PTR CALLBACK RWVUrlDlgProc(HWND h, UINT m, WPARAM w, LPARAM l)
{
  ...
  LogF("[OpenURLDlgLayout] dlgW=%d editX=%d editW=%d pad=%d totalClientH=%d mode=%d", dlgW, editX, newEditW, pad, totalHClient, mode);
      ...
      // Returning FALSE preserves the control selected with SetFocus above.
      // Returning TRUE lets the dialog manager pick the default button, which
      // is why Open URL from the WebView context menu required a second click.
      return FALSE;
    }
    case WM_COMMAND:
    {

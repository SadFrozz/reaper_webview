...
struct WebViewInstanceRecord {
  ...
  bool isRandomInstance = false;  // random instances omit named-instance controls
  bool forwardReaperHotkeys = true; // per-instance gate combined with the global preference
  bool browserHasFocus = false;   // native browser focus, also covers out-of-process child HWNDs
  ...
  bool findFieldFocused = false;     // explicit focus state used by the REAPER hotkey bridge
  bool webContentEditing = false;    // HTML input/textarea/select/contenteditable owns focus

...
// ===================================================================
// WINDOWS РЕАЛИЗАЦИЯ (ВАШ ОРИГИНАЛЬНЫЙ КОД, БЕЗ ИЗМЕНЕНИЙ)
// ===================================================================
#ifdef _WIN32
...
#else
// ===================================================================
// MACOS РЕАЛИЗАЦИЯ (ИСПРАВЛЕННАЯ)
// ===================================================================

@interface SWELLWebView : NSView {
@public
    WKWebView* webView;
}
@end

@implementation SWELLWebView
...
LRESULT SwellWndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        case WM_CREATE: {
            SWELLWebView* ourView = [[[SWELLWebView alloc] initWithFrame:NSMakeRect(0, 0, 100, 100)] autorelease];
            
            WKWebViewConfiguration* config = [[[WKWebViewConfiguration alloc] init] autorelease];
            WKWebView* wv = [[[WKWebView alloc] initWithFrame:[ourView bounds] configuration:config] autorelease];
            [wv setAutoresizingMask:NSViewWidthSizable | NSViewHeightSizable];
            ourView->webView = wv;
            [ourView addSubview:wv];
            
            SWELL_SetWindowLong(hwnd, 0, (LONG_PTR)ourView);
            
            NSView* parentView = (NSView*)SWELL_GetViewForHWND(hwnd);
            [parentView addSubview:ourView];
            [ourView setFrame:[parentView bounds]];
            [ourView setAutoresizingMask:NSViewWidthSizable | NSViewHeightSizable];
            
            char* url_c_str = (char*)((CREATESTRUCT*)lParam)->lpCreateParams;
            if (url_c_str) WEBVIEW_Navigate(url_c_str);
...
void OpenWebViewWindow(const std::string& url) {
    ...
    g_hwnd = CreateWindowEx(0, "MyWebViewSwellClass", "WebView", 0, 0, 0, 800, 600, g_hwndParent, 0, g_hInst, (void*)url_param);
    free(url_param);
}

void WEBVIEW_Navigate(const char* url) {
    if (!g_hwnd) return;
    SWELLWebView* ourView = (SWELLWebView*)SWELL_GetWindowLong(g_hwnd, 0);
    if (!ourView || !ourView->webView || !url) return;
    
    if (strcmp(url, "refresh") == 0) {
        [ourView->webView reload];
        return;
    }
    ...
    @autoreleasepool {
        ...
        [ourView->webView loadRequest:request];
    }
}
#endif
